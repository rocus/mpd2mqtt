...
interprete_mqtt_queue_command()
{
  ...
  load=$( echo "$1" | jq --compact-output --raw-output '.load' )
  loaddir=$( echo "$1" | jq --compact-output --raw-output '.loaddir' )
  ...
  if [ "${searchadd}" != "null"  -a  "${searchadd}" != "" ]
  then
    mpc --host="${mpd_host}" --port="${mpd_port}" "searchadd" "Artist" "${searchadd}"
  fi
  if [ "${load}" != "null"  -a  "${load}" != "" ]
  then
    mpc --host="${mpd_host}" --port="${mpd_port}" "load" "${load}"
  fi
  ...
  then
    ...
    then
      ...
      mpc --host="${mpd_host}" --port="${mpd_port}" "play" "${play}"
    fi
  fi
}
...
{
  log_info "interprete_mqtt_command($1)"
  ...
  log_info "player=${player}   options=${options}  queue=${queue}"
